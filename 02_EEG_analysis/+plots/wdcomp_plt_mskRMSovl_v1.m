% Wet-Dry comparison - Masker Crest Factor errorbar plot (overlay) v1 % 
...
% v1  
% 20250612 Masker Crest Factor errorbar plot (overlay) for EEG Wet-Dry gear comparison
% 20250618 CF -> RMS correction

function wdcomp_plt_mskRMSovl_v1(plot_data, plot_params)
...
%% Masker RMS errorbar plot (overlay)
...
for i = 1:numCh
    ...
    legend(leg_BF, 'Location', 'southeast', 'FontSize', lgnd_fontsize);  % Legend with font size
...
sgtitle(figuretitle)
pdfname = [dirname_fig, 'RMS(extract)_Msk_ovl_', sublist_name, sprintf('_%d%d_%d%d',maxrange*1000, baserange*1000), '.pdf'];
saveas(gcf, pdfname)