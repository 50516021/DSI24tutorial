% RMA calculation for ERP version 1 % 
% - evoked responce
%
% required Add-ons
% - 
...
function [EEG_Msk, EEG_Tgt, rmsMsk, rmsTgt, num_sub_indep, num_sub_inter] = RMScal_v1(nums, sub_dir, subList, EEGopt)
...
for i = 1:Snum
    ...
    if double(subID)>87
        for k = 1:(5-strlength(subID))
            subID = strcat("0", subID); %make the subject number as 5 digit
        end
        if double(subID)<131
            EEGfile_tag = EEGfiletag_options(2);
        else
            EEGfile_tag = EEGfiletag_options(3);
        end
    elseif (subList.ID(i) < 10) || (subList.ID(i) > 34)
        subID = strcat('0', string(subList.ID(i)));
        EEGfile_tag = EEGfiletag_options(1);
    else
        subID = string(subList.ID(i));
        EEGfile_tag = EEGfiletag_options(1);
    end
    ...
    % experiment name %
    folders = struct2table(dir(strcat(sub_dir, 's', subID, '_*')));
    ...
    EEGfile = ls(strcat(sub_dir_tmp, EEGfile_tag)); %find responce file
    ...
    [maxMsk(i,:,:),   maxMsk_ind(i,:,:)]   = max(saveEpMsk(round(fsEEG*(baselinedur+maxrange_Msk(1)):fsEEG*(baselinedur+maxrange_Msk(2))),:,:));
    [maxTgt(i,:,:,:), maxTgt_ind(i,:,:,:)] = max(saveEpTgt(round(fsEEG*(baselinedur+maxrange_Tgt(1)):fsEEG*(baselinedur+maxrange_Tgt(2))),:,:,:));
